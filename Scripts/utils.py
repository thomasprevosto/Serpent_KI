...
def intToString(txt_int):
    ...
    decoded_text = byte_representation.decode('utf-8',errors='replace')
    return decoded_text
#-----------------------------------------------------------------------------------------END BASE FUNCTIONS
#-----------------------------------------------------------------------------------------RSA FUNCTIONS
def parseRSA(cle):
...
def extraire_signature_valeur(text_cert):
    """ Extrait la valeur de la signature d'un fichier de demande de certificat. """
    list = text_cert.split("\n")
    for ligne in list:
        if "Signature Value: " in ligne:
            signature_value = ligne.split(":")[1].lstrip()
            break
    return signature_value

def extraire_field(nom_champ,text_cert):
    list = text_cert.split("\n")
    for ligne in list:
        if nom_champ in ligne:
            value = ligne.split(":")[1].lstrip()
            break
    return value
#-----------------------------------------------------------------------------------------END CERT FUNCTIONS
